...
export default function HomeScreen() {
  const { transactions: rawTransactions, budgets, profile, openBalance } = useAppData()
    ...
    <div>
      ...
      <div className="pt-12 pb-8 border-b border-linen">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-[9px] tracking-[.18em] uppercase text-ash">Balance</span>
          <button
            onClick={openBalance}
            aria-label="Edit balance"
            className="text-ash hover:text-char transition-colors cursor-pointer text-[11px] leading-none p-0.5"
          >
            ✎
          </button>
        </div>
        <div
          className={`font-serif text-[44px] md:text-[64px] font-light leading-none tracking-[-0.02em] mb-2 ${
            runningBalance < 0 ? 'text-brand-red' : 'text-ink'
          }`}
        >
          {runningBalance < 0 ? '−' : ''}{fmt(runningBalance)}
        </div>
        <div className="text-[9px] tracking-[.14em] uppercase text-ash mb-5">
          {MONTH_NAMES[m]} {y}
        </div>
        ...
        <div className="flex items-center gap-4 mb-4">
          {(['day', 'week', 'month'] as const).map(v => (
            <button
              key={v}
              onClick={() => setView(v)}
              className={`text-[9px] tracking-[.16em] uppercase pb-1 border-b transition-colors cursor-pointer ${
          ...
          <button
            ...
            className={`p-1 -mr-1 text-base transition-colors ${
              (view === 'day' ? selectedDay >= todayStr : (view === 'week' ? weekOffset : monthOffset) >= 0)
                ...
                : 'text-ash hover:text-char cursor-pointer'
            }`}
          >
            →
          </button>
